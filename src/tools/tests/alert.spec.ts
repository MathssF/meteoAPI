...
describe('AlertController & AlertService', () => {
  ...
  describe('AlertService', () => {
    it('deve criar um alerta', async () => {
      const dto = { localId: '1', parameterId: '2', min: 10, max: 20 };
    ...
    it('deve buscar triggeredAlerts por alertId', async () => {
      prismaMock.triggeredAlert.findMany.mockResolvedValue([{ id: 't1' }]);
      const result = await service.findTriggered({ alertId: '1' });
      expect(prismaMock.triggeredAlert.findMany).toHaveBeenCalledWith({ where: { alertId: '1' } });
